...
{
	queue<GrammerNode*> nqueue;
	nqueue.push(projectNode);
	while (!nqueue.empty()) {
		GrammerNode* target = nqueue.front();
		nqueue.pop();
		if (target->left)
			nqueue.push(target->left);
		if (target->right)
			nqueue.push(target->right);

		if (target->type == GrammerParameter::CONNECTOR) {
			if (!target->infos.empty() && target->infos[0] == "@natural@")
				return "";//自然连接无法判断为哪一个
		}

		if (target->type == GrammerParameter::FROM) {
			vector<string> attrs;
			attrs = SqlGlobal::getInstance()->getAttribute(target->infos[0]);
