...
void M_DML::SelList::addAttribute(Attribute* attr)
{
	selList.push_back(attr);
}

void M_DML::SelList::showInfo()
{
	cout << "SelList:" << endl;
...
GrammerNode* M_DML::SelList::transform2Grammer()
{
...
void M_DML::FromList::addNode(Relation* node)
{
	fromList.push_back(node);
}

void M_DML::FromList::addNode(Query* node)
{
	queryList.push_back(node);
}
void M_DML::FromList::showInfo()
{
	cout << "fromList:" << endl;
...
GrammerNode* M_DML::FromList::transform2Grammer()
{
...
void M_DML::OrderList::addOrderNode(OrderNode* ornode)
{
	orderList.push_back(ornode);
}

void M_DML::OrderList::showInfo()
{
	cout << "orderList:" << endl;
...
GrammerNode* M_DML::OrderList::transform2Grammer()
{
	GrammerNode* gnode = new GrammerNode();
...
void M_DML::GroupList::addAttribute(Attribute* attr)
{
	groupList.push_back(attr);
}

GrammerNode* M_DML::GroupList::transform2Grammer()
{
	return NULL;
}

void M_DML::Attribute::setName(string attrName, string tableName)
{
	this->attrName = attrName;
	this->tableName = tableName;
}

void M_DML::Attribute::setName(string name)
{
...
void M_DML::Attribute::setName(Name* name)
{
...
void M_DML::Attribute::showInfo()
{
	cout << "attribute:" << attrName << endl;
	if (tableName != "")
		cout << "tableName:" << tableName << endl;
	if (aggFunc != "")
		cout << "aggFunc:" << aggFunc << endl;
}

string M_DML::Attribute::getAttributes()
{
	string message = "";
	if (tableName != "") {
		//存在表名,a.b形式
...
void M_DML::Number::showInfo()
{
	cout << "Number:" << number << endl;
}

void M_DML::OrderNode::showInfo()
{
	cout << "OrderNode:" << endl << "rule:" << rule << endl;
	attr.showInfo();
}

void M_DML::Relation::showInfo()
{
	cout << "Relation:" << relationName << endl;
}

void M_DML::Operator::showInfo()
{
	cout << "Operator:" << opName << endl;
}

void M_DML::Condition::showInfo()
{
	cout << "Condition:" << endl;
	if (Lattr != nullptr) {
		cout << "Lattr:" << endl;
...
GrammerNode* M_DML::Condition::transform2Grammer()
{
...
string M_DML::Condition::getInfo()
{
	string info = "";
	if (rNumber != nullptr) {
		//说明右操作数为数字
...
string M_DML::Condition::getLoopInfo()
{
	if (lCnode == nullptr) {
		//说明为叶节点
		return getInfo();
...
string M_DML::Condition::setGrammerVec(vector<string>& infos, int count, int leafCount)
{
	if (leafCount == 0) {
		//说明只有一个节点
		if (query == nullptr) {
...
vector<M_DML::Condition*>* M_DML::Condition::getSubQueryCondition()
{
...
GrammerNode* M_DML::Query::transform2Grammer()
{
	GrammerNode* gnode;
	if (condition) {
...
GrammerNode* M_DML::Query::transform2GrammerWithSubQuery(vector<Condition*>* conditionWithSub)
{
...
GrammerNode* M_DML::Query::transform2GrammerWithSimple()
{
	GrammerNode* root, * ptr;
...
void M_DML::Query::turnCondition2Connect(GrammerNode* sroot, string condition)
{
...
void M_DML::ParaString::showInfo()
{
	cout << "String_parameter:" <<parameter << endl;
}

void M_DML::Insert::showInfo()
{
	cout << "Insert:" << endl;
	cout << "tableName:" << tableName << endl;
...
GrammerNode* M_DML::Insert::transform2Grammer()
{
	GrammerNode* gnode = new GrammerNode();
...
GrammerNode* M_DML::Delete::transform2Grammer()
{
	//创建根结点
	GrammerNode* root = new GrammerNode();
...
GrammerNode* M_DML::Update::transform2Grammer()
{
	//结点info格式：属性 值 属性 值...
	GrammerNode* root = new GrammerNode();
	map<string, string>::iterator iter = attr_indexs.begin();
...
GrammerNode* M_DML::Node::transform2Grammer()
{
	return nullptr;
}
